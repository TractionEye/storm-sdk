...
export function createStormTools(client) {
    return [
        ...
        },
        {
            ...
            description: 'Get positions grouped by token. Shows total amount, total TON invested, realized PnL, and entry price per asset. Useful for portfolio overview.',
            parameters: { type: 'object', properties: {}, additionalProperties: false },
            handler: async () => {
                try {
        ...
        },
        {
            ...
            description: 'Check the status of an async deal after opening or closing a position. Statuses: "pending" = waiting for on-chain settlement, "confirmed" = done, "failed" = error (check error field), "adjusted" = confirmed but amounts differ from expected.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Close an existing futures position (fully or partially). Provide the deal ID and amount in nanoTON. For full close, use the full margin amount of the position. Returns status "pending" — use storm_wait_confirmation to wait for on-chain settlement.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Create a Take Profit or Stop Loss order for an existing open position. Specify order_type ("takeProfit" or "stopLoss"), base asset (e.g. "BTC"), direction of the position, margin in nanoTON, order size in nanoTON, and trigger price in nanoTON. IMPORTANT: triggerPrice is in nanoTON, NOT USD.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Wait for a deal to be confirmed on-chain. Polls status every 3 seconds until the deal reaches a terminal state (confirmed/failed/adjusted) or times out after 120 seconds. Use after storm_open_position or storm_close_position. If you prefer manual polling, use storm_get_swap_status instead.',
            parameters: {
                type: 'object',
                properties: {
